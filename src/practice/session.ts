...
export type PracticePhase = 'setup' | 'intro' | 'question' | 'feedback' | 'complete'
...
export function readPracticeSession(): SavedPracticeSession {
  ...
  const phase = saved.phase === 'intro' || saved.phase === 'setup' || saved.phase === 'question' || saved.phase === 'feedback' || saved.phase === 'complete'
    ? saved.phase
    : 'setup'
  return {
